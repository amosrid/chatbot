...
class WP_DeepSeek_Chatbot {
    ...
    private function init_hooks() {
        ...
        add_action('wp_footer', array($this, 'render_fab_menu'));
    }
...
public function render_chatbot() {
    ...
    // Check if FAB menu is enabled - if enabled, don't show chatbot here at all
    $enable_fab_menu = isset($settings['enable_fab_menu']) ? $settings['enable_fab_menu'] : 0;
    if ($enable_fab_menu) {
        return; // Don't render anything if FAB is enabled
    }
 ...
 * Render FAB menu
 */
public function render_fab_menu() {
    $settings = get_option('wp_deepseek_chatbot_settings', array());
    
    // Check if FAB menu is enabled
    $enable_fab_menu = isset($settings['enable_fab_menu']) ? $settings['enable_fab_menu'] : 0;
    if (!$enable_fab_menu) {
        return;
    }
    ...
    // Get menu items
    $menu_items = isset($settings['fab_menu_items']) ? $settings['fab_menu_items'] : array();
    
    // Get agent name for chat option
    $agent_name = isset($settings['agent_name']) ? $settings['agent_name'] : __('AI Assistant', 'wp-deepseek-chatbot');
    
    ?>
    <div class="wp-deepseek-fab-container">
        <div class="wp-deepseek-fab-toggle">
            <i class="dashicons dashicons-plus"></i>
        </div>
        <div class="wp-deepseek-fab-menu">
            <div class="wp-deepseek-fab-items">
                <?php 
                // Display menu items
                foreach ($menu_items as $item) :
                    if (empty($item['name']) || empty($item['whatsapp_number'])) continue;
                    
                    // Prepare WhatsApp link
                    $whatsapp_number = preg_replace('/[^0-9]/', '', $item['whatsapp_number']);
                    $whatsapp_message = urlencode($item['whatsapp_message']);
                    $whatsapp_link = "https://wa.me/{$whatsapp_number}?text={$whatsapp_message}";
                ?>
                    <a href="<?php echo esc_url($whatsapp_link); ?>" target="_blank" class="wp-deepseek-fab-item">
                        <div class="wp-deepseek-fab-item-icon">
                        ...
                        <div class="wp-deepseek-fab-item-text"><?php echo esc_html($item['name']); ?></div>
                    </a>
                <?php endforeach; ?>
                
                <!-- Chat option always shown at the bottom -->
                <a href="#" class="wp-deepseek-fab-item wp-deepseek-fab-chat-option" id="wp-deepseek-toggle-chatbot">
                    <div class="wp-deepseek-fab-item-icon">
                        <i class="dashicons dashicons-format-chat"></i>
                    </div>
                    <div class="wp-deepseek-fab-item-text"><?php printf(__('Chat dengan %s', 'wp-deepseek-chatbot'), esc_html($agent_name)); ?></div>
                </a>
            </div>
        </div>
        
        <!-- Include the chatbot widget directly within the FAB container when FAB is enabled -->
        <div id="wp-deepseek-chatbot-widget" class="wp-deepseek-chatbot-widget">
            <div class="wp-deepseek-chatbot-header">
                <span class="wp-deepseek-chatbot-title"><?php echo esc_html($agent_name); ?></span>
                <span class="wp-deepseek-chatbot-close">&times;</span>
...
public function enqueue_frontend_assets() {
    ...
    wp_localize_script('wp-deepseek-chatbot-script', 'wpDeepseekChatbot', array(
        ...
        'fab_enabled' => $enable_fab_menu, // Tambahkan ini
        'strings' => array(
    ...
    // Tambahkan class body jika FAB diaktifkan
    if ($enable_fab_menu) {
        add_filter('body_class', function($classes) {
            $classes[] = 'wp-deepseek-fab-menu-enabled';
            return $classes;
        });
    }
}
